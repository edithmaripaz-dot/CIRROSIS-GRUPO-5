...
-   dias_seguimiento: tiempo de supervivencia observado en días.
-   edad: Indica la edad de los pacientes.
...
```{r}
cirrosis_4 |> 
  select(dias_seguimiento, cobre, etapa) |> 
  summary()
```
...
```{r}
Surv(cirrosis_4$edad,cirrosis_4$trigliceridos)
```

```{r}
km = survfit(Surv(dias_seguimiento, bilirrubina) ~ 1, data = cirrosis_4)
...
```{r}
survfit2(Surv(dias_seguimiento, bilirrubina) ~ 1, data = cirrosis_4) |>  
  ggsurvfit() +
  labs(
    x = "Días",
...
```{r}
summary(survfit(Surv(dias_seguimiento, bilirrubina) ~ 1, data = cirrosis_4), times = 365.25)
```
...
```{r}
survfit(Surv(dias_seguimiento, bilirrubina) ~ 1, data = cirrosis_4) %>% 
  tbl_survfit(
    times = 365.25,
...
```{r}
survdiff(Surv(dias_seguimiento, bilirrubina) ~ estado, data = cirrosis_4)
```
...
```{r}
survfit2(Surv(dias_seguimiento, bilirrubina) ~ medicamento, data = cirrosis_4) |>
  ggsurvfit() +
  labs(
    x = "Días",
...
#Importar CSV
datos <- read.csv("D:/ALFRE/UPSJB 2025/2025-2/R STUDIO/ESTADISTICA/data_ESTUDIANTE/cirrosis/cirrosis.csv", header = TRUE, sep = ",")

# Importar con readr
library(readr)
datos <- read_csv("D:/ALFRE/UPSJB 2025/2025-2/R STUDIO/ESTADISTICA/data_ESTUDIANTE/cirrosis/cirrosis.csv")
...
datos <- datos[, colMeans(is.na(datos)) < 0.5]
...
```{r}
datos$Plaquetas <- scale(datos$Plaquetas)
```
...
datos <- datos[datos$Dias_Seguimiento > 78, ]
names(datos)[1] <- "recien ingresado"
...
ggplot(datos, aes(x = Dias_Seguimiento)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "#69b3a2", color = "white") +
  geom_density(color = "red", size = 1) +
  labs(title = "Distribución de Dias Seguimiento", x = "HORAS EN DIAS", y = "Densidad") +
  theme_minimal()


```

**Paso 4: Pruebas Inferenciales**

#### **Chi-cuadrado para variables categóricas**:

```{r}

tabla <- table(datos$Trigliceridos, datos$Tiempo_Protrombina)
chisq.test(tabla)
