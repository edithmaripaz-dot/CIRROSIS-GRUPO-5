...
```{r}
plot(ID ~ Colesterol , data = cirrosis,
     col = "gray",
     ylab = "Leucocitos",
     xlab = "edad",
     las = 1,
...
abline(lm(ID ~ Colesterol , data = cirrosis), lwd = 2, col = "brown")
```
...
```{r}
group_by(cirrosis, Edema) |> 
  summarise(
    count = n(),
    mean = mean(Ascitis, na.rm = TRUE),
    sd = sd(Colesterol, na.rm = TRUE)
  )
```
...
```{r}
cirrosis |>  
  filter(!is.na(Colesterol)& !is.na(Edema))  |> 
  ggplot(aes(y = Colesterol, x = Edema)) +
  geom_boxplot() +
  labs(y = "nivel de colesterol", x = "tipo de edema")
```
