...
```{r}
data_cirrosis_tiempo <- import(here("cirrosis.csv"))
```
...
```{r}
shapiro.test(data_cirrosis_tiempo$Tiempo_Protrombina)
```

## Para la variable Edad

Esta variable corresponde a medidas de edad x dias

```{r}
shapiro.test(data_cirrosis_tiempo$Edad)
```
...
```{r}
table(data_cirrosis_tiempo$Ascitis)
```
